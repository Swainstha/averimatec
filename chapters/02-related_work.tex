...
Ranking fourth,~\citet{Tsoneva2026Mar} proposed \textit{Xxp} system which treats multimodality as a controllable design variable. It decomposes claims into specific "text-related" or "image-related" questions and applies targeted retrieval strategies for each. A key philosophical choice in this architecture is the reliance on text-only evidence for the final verdict, based on the hypothesis that textual grounding is currently more stable for veracity assessment.
...
Xxp ~\cite{Tsoneva2026Mar} also uses SigLIP(siglip-base-patch16-384) for a separate multimodal pipeline. It creates a joint embedding of the textual query and the claim image, then computes cosine similarity against token-aware chunks in an image-related knowledge store. ADA-AGGR employs ColPali 1.3, which utilizes a "late-interaction" mechanism to compare textual queries against multimodal image patches. It uses these embeddings for both "Image Retrieval" (visual-only) and "Multimodal Retrieval" (text + image queries). VILLAIN uses the Ops-MM-embedding-v1-7B multimodal model
. It queries this store using both the claim text and the claim images to retrieve the top-ranked visual evidence.
...
Xxp uses the gte-base embedding model to encode the query and the top 2,000 segments after the sparse BM25 pass. It then computes cosine similarity to rank these candidates and selects the top 10 most similar segments. ADA-AGGR re-ranked the 1000 candidates retrieved by BM-25 using a dense model—typically Qwen3-4B-Embedding or Qwen3-E-0.6B—to select the top five pieces of text evidence. VILLAIN applies a secondary dense stage using mxbai-rerank-large-v1 to narrow the top 100 candidates down to the top 10 most relevant segments ~\cite{Tsoneva2026Mar}.
...
After the introduction of the multimodal AVERIMATEC dataset  ~\cite{Cao2025May} , most of the competitive system have employed MLLMs to explore the multimodal nature of the claims and reasoning over the retrieved multimodal evidences.
VILLAIN used Gemini-2.5-Pro for all vision-language model (VLM) agents and showed it outperformed all other models evaluated (including Gemini-2.5-Flash and the entire Qwen3-VL series) across nearly all metrics in its ablation studies. ADA-AGGR used Gemini 3 Pro and found that Gemini 3 Pro enabled the highest veracity accuracy (0.447) compared to Qwen3-VL-8B variants and Gemini 2.5 Flash. XxP, utilized the open-weight Qwen3-VL-8B-Instruct model across all stages of its selective multimodal pipeline. AIC CTU employed GPT-5.1 as the backbone of their generation module.
...
Gemini-2.0-Flash-001, Qwen2.5-VL-7B-Instruct, Gemma-3-12B and LLaVA-Next-7B ~\cite{Jung2026Mar} ~\cite{Upravitelev2026Mar} ~\cite{Tsoneva2026Mar} ~\cite{Ullrich2026Mar} ~\cite{Cao2025May}.
