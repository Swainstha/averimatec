...
The FEVER framework is characterized by three main labels: SUPPORTED, REFUTED, and NOT ENOUGH INFO. In addition to predicting the correct label, systems are required to retrieve sentence-level evidence that justifies the verdict. Many claims also require multi-hop reasoning, where evidence must be combined from multiple sentences or documents.
...
~\citet{Tsoneva2026Mar} proposed Xxp system which treats multimodality as a controllable design variable. It decomposes claims into specific text-related or image-related questions and applies targeted retrieval strategies for each. A key philosophical choice in this architecture is the reliance on text-only evidence for the final verdict, based on the hypothesis that textual grounding is currently more stable for veracity assessment.

~\citet{Upravitelev2026Mar} proposed ADA-AGGR framework which emphasizes breadth in evidence gathering by running multiple retrieval methods—textual, image-only, and multimodal—simultaneously for every claim
