...
\section{Hybrid Multimodal Retrieval Augmented Generation}
\label{ch:rag}

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{images/NewNewRag.pdf}
\caption{Hybrid Multimodal Retrieval Augmented Generation. The yellow sections are the inputs and red sections are the outputs of RAG. The purple sections are the different indexes for the specific retrieval methods}
\label{fig:rag}
\end{figure}

We have a dual RAG system as shown in Figure \ref{fig:rag} where different retrieval methods are used to retrieve different information from different sources. For retrieving information for the claim text Splade and BGE are used and the information is retrieved from the text related text documents based on the claim text, its paraphrases and the fact checking question if available. Different queries are built for splade and BGE retrieval systems.

Similarly for image related retrievals, we have the claim images, their corresponding captions  and fact checking question if available . The claim Image, caption and the question is used to retreive the related text and image documents from the image related documents  given in the dataset and also from the other source which was retrieved using Reverse image search using Google API using the LongCLIP retreival.
...
In our implementation, cross-encoders are only used to rerank the text documents retrieved by BGE and splade and not used for documents retrieved by LongCLIP. Since the BGE-reranker is a text only reranker, we have avoided its use for image related documents as they depend on the claim images rather than the claim text.

\subsection{Maximum Marginal Relevance (MMR)}

After reranking, the top 20 documents are further processed using Maximum Marginal Relevance (MMR) to ensure diversity in the retrieved evidence. 
...
This whole pipeline of evidence retrieval will be denoted as 'Evidence Retrieval using RAG' in the flow diagrams in next sections.
