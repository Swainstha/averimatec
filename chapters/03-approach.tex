...
Dense retrieval encodes text into continuous vector representations, enabling semantic matching beyond exact keywords. This work uses the BAAI/bge-large-en-v1.5 model as the primary dense retriever, allowing retrieval of semantically related evidence even when wording differs significantly. For example, a claim about “a 101-year-old woman giving birth” can retrieve evidence discussing “extreme-age pregnancy hoaxes” despite lexical differences.


\subsection{Sparse Lexical Retrieval using Splade}
To complement dense retrieval, the system employs SPLADE, a sparse neural retrieval model that combines lexical matching with learned term expansion. SPLADE bridges traditional sparse methods such as BM25 with modern dense retrieval, providing effective and computationally efficient sparse representations ~\cite{Formal2021Sep}.
...
Within each retrieval branch, the ranked outputs are combined using Reciprocal Rank Fusion (RRF).
...
RRF accumulates the scores for documents that appear in more than one list. This ensures that documents found to be relevant across multiple "angles" of a query receive the highest priority in the final ranking ~\cite{Rackauckas2024Jan}. In the text-based branch, RRF fuses BGE and SPLADE ranked outputs. In the image-based branch, RRF fuses the ranked text evidence streams returned by LongCLIP retrieval sources.
...
In our implementation, cross-encoders are used only for textual evidence reranking after RRF for the text branch. They are not applied to image branch because the BGE-reranker is a text-only reranker. This keeps visual evidence handling in the image branch while still improving precision for the final text evidence list.
...
The final output of this pipeline is a ranked list of the top 20 most relevant and diverse text evidence passages, along with any image evidence retrieved through LongCLIP's cross-modal search for image related retrieval.
...
The final prompt is assembled by combining all processed inputs into a structured template that guides the VLM toward producing diagnostic question-answer pairs. The prompt consists of:
