...
\begin{itemize}
    ...
    \item \textbf{Heterogenous information signals} - Relevant evidence may be distributed across textual content, images, OCR text, and metadata, requiring unified multimodal retrieval strategies.
\end{itemize}
...
\begin{itemize}
    \item claim text
    \item Paraphrases of the claim text if available
    \item generated verification questions if available
\end{itemize}
...
Standard CLIP is limited to 77 tokens, with an effective context length of about 20 tokens, making it less effective for long contextual evidence and fine-grained attribute interactions. LongCLIP extends CLIP-based image–text alignment to support longer textual contexts, improving retrieval for descriptive evidence, image-associated narratives, and multimodal misinformation through an extended text encoder and dual-alignment fine-tuning.

By using Knowledge-Preserved stretching and primary component matching, LongCLIP improves recall by approximately 25\% on long-text retrieval tasks and 6\% on traditional short-text retrieval, all while maintaining original zero-shot classification performance~\cite{Zhang2024Mar}.
...
In this thesis, we refer to our Hybrid Multimodal Dual-Branch Retriever as Dual-RAG. Dual-RAG separates evidence retrieval into text-based and image-based branches. The objective of this separation is to preserve strong lexical and semantic signals from claim text while also capturing cross-modal visual evidence from claim images.
...
To guide the QA generation model toward producing high-quality verification questions, the system selects the top-5 most similar training claims as in-context learning (ICL) demonstrations. The selection uses dense semantic similarity via the \textit{all-MiniLM-L6-v2} sentence transformer:
