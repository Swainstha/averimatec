...
Automated fact-checking is a complex task whose objective is to determine the veracity of a claim by retrieving relevant evidence from large information sources and analyzing the relationship between the claim and the retrieved evidence.
...
Later, ~\citet{Thorne2018Jun} introduced \textit{FEVER (Fact Extraction and VERification)}, a large-scale benchmark for automated fact-checking consisting of 185,445 claims verified against Wikipedia. Unlike earlier approaches, FEVER introduced a retrieval-based verification framework in which systems must retrieve relevant evidence before determining claim veracity.

The FEVER framework was characterized by three main labels: SUPPORTED, REFUTED, and NOT ENOUGH INFO. In addition to predicting the correct label, systems were required to retrieve sentence-level evidence that justifies the verdict. Many claims also required multi-hop reasoning, where evidence must be combined from multiple sentences or documents.
...
However this dataset contain purpose-made claims derived from Wikipedia, and thus are not representative of the real-world misinformation that circulates online. The claims in FEVER were artificially generated by altering sentences from Wikipedia, which may not capture the complexity, nuance, and multimodal nature of real-world misinformation.

To address these limitations, ~\citet{Schlichtkrull2023May} introduced AVERITEC, a fact-checking dataset containing 4,568 real-world claims collected from professional fact-checking organizations. Unlike the initial FEVER, AVERITEC incorporated realistic web-retrieved evidence, structured question-answer based retrieval, and detailed justifications explaining how evidence supports or refutes a claim. The dataset also avoided common issues such as context dependence, insufficient evidence, and temporal leakage, making it more suitable for practical fact-checking research.

However, both FEVER and AVERITEC primarily focused on textual verification, whereas real-world misinformation frequently involves multimodal content such as images, videos, and screenshots. This limitation had motivated the development of multimodal fact-checking approaches that jointly analyze textual and visual information.
...
To support research in multimodal fact-checking, ~\citet{Cao2025May} introduced AVerImaTeC  (Automatic Verification of Image-Text Claims), a dataset containing 1,297 real-world multimodal claims composed of both images and text in the FEVER 9 workshop. Each claim was annotated with question-answer evidence pairs retrieved from the web, along with metadata, veracity labels, and textual justifications explaining how the final verdict is derived.
...
By explicitly incorporating evidence retrieval and structured reasoning, this approach improved interpretability and provides evidence-based justifications for its predictions, addressing key limitations of earlier multimodal fact-checking methods.
...
~\citet{Tsoneva2026Mar} proposed Xxp system which treated multimodality as a controllable design variable. It decomposes claims into specific text-related or image-related questions and applies targeted retrieval strategies for each. A key philosophical choice in this architecture is the reliance on text-only evidence for the final verdict, based on the hypothesis that textual grounding is currently more stable for veracity assessment.

~\citet{Upravitelev2026Mar} proposed ADA-AGGR framework which emphasized breadth in evidence gathering by running multiple retrieval methods—textual, image-only, and multimodal—simultaneously for every claim
...
~\citet{Ullrich2026Mar} proposed AIC CTU system which prioritized simplicity and cost-effectiveness, utilizing a modular RAG pipeline that is easily reproducible. It relies on a dual-retriever approach that combines offline textual search with real-time Reverse Image Search (RIS), achieving competitive performance with minimal computational overhead.

~\citet{Jung2026Mar} proposed VILLAIN which utilizes a sophisticated multi-agent architecture where specialized VLM agents operate across sequential stages. It focuses on knowledge enrichment by proactively crawling the web to fill missing data and uses dedicated agents to perform modality-specific and cross-modal analyses to resolve inconsistencies among evidences.
...
In essence, ~\citet{Lewis2020May} discusses Retrieval-Augmented Generation (RAG) as a grounding mechanism that reduces reliance on the model’s internal parametric knowledge and encourages evidence-based generation. So RAG has become a central component in modern fact-checking pipelines, especially in the context of FEVER-style tasks, where the system must retrieve evidence from a large corpus and use it to verify claims.

~\citet{Lewis2020May} applied the Retrieval-Augmented Generation (RAG) framework to automated fact-checking on the FEVER dataset introduced by ~\citet{Thorne2018Jun}. In this approach, the system retrieves relevant evidence from Wikipedia and then reasons over the retrieved information to determine the veracity of a claim. Despite not relying on complex retrieval pipelines or intermediate retrieval supervision, RAG achieved competitive performance. For the three-way FEVER classification task, its results were within 4.3\% of state-of-the-art systems that used specialized architectures and extensive engineering.

Both the FEVER baselines presented by ~\citet{Schlichtkrull2023May} and ~\citet{Schlichtkrull2024Nov} utilized Google Search APIs and offline knowledge stores to retrieve evidence documents for claim verification. The retrieved evidence was then used by transformer-based models to reason about the claim and generate a verdict. Similarly all the top frameworks in the AVerImaTec competition deployed RAG as a central component in their pipeline ~\cite{Cao2026Mar}.
...
Empirically, DPR significantly outperformed BM25, improving top-20 retrieval accuracy by 9\%–19\% on several benchmarks ~\citep{Karpukhin2020Nov}. Consequently, modern fact-checking and FEVER-style systems commonly combine dense vector retrieval approaches with traditional sparse retrieval techniques such as BM25 ~\citet{Schlichtkrull2024Nov}. Recent multimodal fact-checking frameworks such as AIC CTU ~\cite{Ullrich2026Mar} and VILLAIN~\cite{Jung2026Mar} also employ mxbai-embed-large-v1 as the dense embedding model. AIC CTU chunks the offline knowledge store and embeds each segment using  mxbai-embed-large-v1 and then performs an exact k-Nearest Neighbour (k-NN) search using the FAISS library to find the most similar segments for textual retrieval ~\cite{Ullrich2026Mar}. Similarly, VILLAIN uses the same embedding model mxbai-embed-large-v1 to find the top 100 candidates from the knowledge store ~\cite{Jung2026Mar}.
...
REVEAL used SFR-Embedding-2\_R to re-score the top 500 candidates from the sparse retrieval stage to filter out irrelevant matches and refine to a final set of 15 textual documents ~\cite{Tariq2026Mar}. Xxp used gte-base embedding model to encode the query and the top 2,000 segments after the sparse BM25 pass. It then computed cosine similarity to rank these candidates and selects the top 10 most similar segments ~\cite{Tsoneva2026Mar}. ADA-AGGR re-ranked the 1000 candidates retrieved by BM-25 using a dense model—typically Qwen3-4B-Embedding or Qwen3-E-0.6B, to select the top five pieces of text evidence ~\cite{Upravitelev2026Mar}. VILLAIN applies a secondary dense stage using mxbai-rerank-large-v1 to narrow the top 100 candidates down to the top 10 most relevant segments ~\cite{Jung2026Mar}.
...
VILLAIN used Gemini-2.5-Pro for all vision-language model (VLM) agents and showed it outperformed all other models evaluated (including Gemini-2.5-Flash and the entire Qwen3-VL series) across nearly all metrics in its ablation studies ~\cite{Jung2026Mar}. ADA-AGGR used Gemini 3 Pro and found that Gemini 3 Pro enabled the highest veracity accuracy (0.447) compared to Qwen3-VL-8B variants and Gemini 2.5 Flash ~\cite{Upravitelev2026Mar}. XxP, utilized the open-weight Qwen3-VL-8B-Instruct model across all stages of its selective multimodal pipeline ~\cite{Tsoneva2026Mar}. AIC CTU employed GPT-5.1 as the backbone of their generation module ~\cite{Ullrich2026Mar}.The REVEAL framework utilized Qwen2.5-VL (specifically the Qwen2.5-VL-7B-Instruct variant) and Team fv utilized Qwen3-VL-32B as its central backbone for multimodal reasoning and evidence processing ~\cite{Tariq2026Mar}. The baseline tested several combinations of Large Language Models (LLMs) like Qwen2.5-7B-Instruct and Multimodal Large Language Models (MLLMs) like 
...
Xxp prompts the MLLM to generate 3 to 5 verification questions which are either labeled text-related or image-related. These questions trigger modality-aware retrieval strategies to retrieve the evidences and answer the questions ~\cite{Tsoneva2026Mar}. Similar question generation strategy was used by Team fv as well.

The REVEAL framework uses an iterative, history-aware question generation strategy, where each question is generated based on previously retrieved evidence. Questions are classified as either text-related or image-related to guide retrieval, and the process continues until sufficient evidence is gathered to reach a verdict. While effective, the sequential nature of this approach increases computational cost ~\cite{Tariq2026Mar}.
