...
Each claim is associated with a knowledge store containing pre-collected documents from which evidence must be retrieved. The knowledge store is organized into three modalities: (1)~\textit{Text} documents, consisting of web pages retrieved by search engines for the claim; (2)~\textit{Image} documents, consisting of text extracted from web pages found via reverse image search on the claim image; and (3)~\textit{External} documents, which are additional web pages retrieved through extended reverse image search. Table~\ref{tab:doc_stats} presents the document statistics.
...
Validation & External & 2,371   & 0 & 31,545    & 482   \\
Test       & Text     & 503,006 & 1 & 2,893,192 & 5,214 \\
Test       & Image    & 7,814   & 1 & 51,689    & 561   \\
Test       & External & 9,717   & 0 & 51,689    & 451   \\
\hline
\end{tabular}
\end{table}
...
where $N$ is the total number of evaluated claims and $\mathbb{1}[\cdot]$ is the indicator function. This metric ranges from 0 to 1, with 1 indicating perfect classification across all four verdict categories.
...
\subsection{RQ1: Contribution of Retrieval Components}
...
With Cross Encoder        & 0.42 & 0.66 & 0.41 & 0.40 & 0.32 \\
With MMR                  & 0.45 & 0.71 & 0.44 & 0.46 & 0.35 \\
\hline
\end{tabular}
\end{table}
...
The cumulative effect of the full pipeline (SPLADE+BGE+LongCLIP+Cross-Encoder+MMR) yields a 32\% relative improvement in evidence score over the SPLADE-only baseline (0.45 vs.\ 0.34) and a 59\% improvement in evidence-conditioned justification score (0.35 vs.\ 0.22).
...
SPLADE+BGE+LongCLIP & 0.495 & 0.224 & 0.23 & 0.17 \\
\hline
\end{tabular}
\end{table}

\subsection{RQ2: Iterative vs.\ Retrieval First QA Generation}

To answer \textbf{RQ2}, we compare two QA generation strategies: (1)~iterative question generation, where questions are generated iteratively followed by evidence retrieval and answer generation ; and (2)~retrieval first QA generation, where all questions and answers are generated in a single VLM call after retrieving evidence. Both configurations use the full retrieval pipeline (SPLADE+BGE+LongCLIP+Cross-Encoder+MMR) and in-context learning.

Table~\ref{tab:icl_ablation} shows the iterative approach on the validation set, while Table~\ref{tab:one_go_val} shows the retrieval first approach.
...
Yes & 0.34 & 0.214 & 0.724 & 0.363  & 0.26 & 0.19 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Retrieval First QA generation pipeline performance on the validation set.}
\label{tab:one_go_val}
\begin{tabular}{lcccccc}
\hline
\textbf{\makecell{In-context \\Learning}} & \textbf{\makecell{Question\\ Score}} & \textbf{\makecell{Evidence\\ Score}} & \textbf{\makecell{Verdict\\Score}} & \textbf{\makecell{Justification\\ Score}} & \textbf{\makecell{Evidence\\ Verdict}} & \textbf{\makecell{Evidence\\Justification}} \\
\hline
No  & 0.35 & 0.237 & 0.81 & 0.42 & 0.29 & 0.22 \\
Yes & 0.4 & 0.23 & 0.77 & 0.37  & 0.28 & 0.22 \\
\hline
\end{tabular}
\end{table}

Comparing the best configuration of each strategy (iterative with ICL vs.\ retrieval first without ICL), the single-pass approach substantially outperforms iterative generation across all metrics: question score (0.35 vs.\ 0.34), evidence score (0.237 vs.\ 0.214), verdict score (0.81 vs.\ 0.72), and justification score (0.42 vs.\ 0.363).
...
On the test set (Table~\ref{tab:one_go_test}), the retrieval first approach with ICL achieves a question score of 0.622, evidence score of 0.312, verdict score of 0.327, and justification score of 0.297—representing our best test set submission. Notably, ICL improves all metrics on the test set despite hurting validation performance, suggesting that the few-shot examples provide better generalization to unseen claims even when they constrain validation-set verdict accuracy.

\begin{table}[ht]
\centering
\caption{Retrieval first QA generation pipeline performance on the test set.}
...
Yes & 0.622 & 0.312 & 0.327 & 0.297\\
\hline
\end{tabular}
\end{table}


\subsection{RQ3: Effect of In-Context Learning}

To answer \textbf{RQ3}, we examine the impact of in-context learning (ICL) on both generation strategies. In the ICL setting, BM25 is used to retrieve similar QA-pair examples from the training set, which are included in the VLM prompt as few-shot demonstrations.
...
For \textit{Retrieval First QA generation} (Table~\ref{tab:one_go_val}), the effect of ICL is mixed on the validation set. While question score improves from 0.35 to 0.4, the verdict score drops from 0.81 to 0.77 and justification score from 0.42 to 0.37. This suggests that in the retrieval first setting, few-shot examples may constrain the VLM's reasoning, causing it to mimic the structure of training examples rather than adapting to the specific claim. The retrieved ICL examples may introduce noise when the test claim differs substantially from the training distribution.

On the test set (Table~\ref{tab:one_go_test}), the pattern reverses: ICL improves all metrics, with evidence score rising from 0.277 to 0.312 (+13\%), verdict score from 0.289 to 0.327 (+13\%), and justification score from 0.265 to 0.297 (+12\%). This reversal suggests that while ICL constrains the VLM on the heavily imbalanced validation set (where unconstrained reasoning exploits majority-class bias), it provides valuable structural guidance on the more diverse test distribution, helping the model generate better-formed questions and more grounded answers.
...
\caption{Confusion matrices for verdict prediction using the single-pass (retrieval-first) QA generation strategy on the validation set. Left: with in-context learning. Right: without in-context learning.}
\label{fig:confusion_matrix_retrieval_first}
\end{figure}

\subsection{RQ4: Comparison with State-of-the-Art Systems}
...
\begin{tabular}{lcccc}
\hline
\textbf{Team} & \textbf{Question Score} & \textbf{Evidence Score} & \textbf{Verdict Score} & \textbf{Justification Score} \\
\hline
HUMANE  & 0.9228 & 0.5832 & 0.6447 & 0.5427 \\
ADA-AGGR  & 0.3539 & 0.3864 & 0.4539 & 0.3727 \\
AIC CTU    & 0.8223 & 0.3466 & 0.375 & 0.3035 \\
Ours (Best)     & 0.35 & 0.237 & 0.81 & 0.42 \\
...
Across all experimental configurations, evidence retrieval score consistently remains the weakest component of the pipeline. Even our best configuration achieves only 0.312 evidence score on the test set, compared to 0.622 for question generation and 0.327 for verdict prediction. This pattern is consistent across competing systems: even the top-ranked HUMANE system achieves only 0.536 evidence score despite scoring 0.890 on question generation.
...
A key design decision in our pipeline is the use of Qwen3-VL-8B-Instruct, an open-source 8B-parameter VLM, while competing systems rely on proprietary MLLMs such as GPT-5.1 and Gemini-2.5-Pro. This choice involves a clear trade-off:
...
\begin{itemize}
    ...
    \item \textbf{Disadvantages:} Lower question generation diversity (0.622 vs.\ 0.890 for HUMANE), reduced capacity for complex multi-hop reasoning, and smaller context window limiting the amount of evidence that can be processed simultaneously.
\end{itemize}
...
One of the clearest findings from RQ1 is the outsized impact of MMR diversification on downstream performance. MMR improved verdict score by 5 percentage points (0.66 to 0.71) on the validation set—the largest single-stage gain in the pipeline. This suggests that providing the VLM with diverse, non-redundant evidence is more important than providing the highest-scoring evidence alone.
...
The val-to-test gap (0.81 to 0.327 verdict score) reveals a cautionary lesson about evaluation on small, imbalanced datasets. The 152-claim validation set with 95\% Refuted labels creates conditions where a system can achieve high accuracy through implicit bias rather than genuine reasoning. The test set, with its larger size and potentially more balanced distribution, provides a more realistic assessment of system capability.
...
\begin{itemize}
    ...
    \item \textbf{Class imbalance:} The extreme imbalance (95\% Refuted) in the known splits makes it difficult to assess performance on minority classes. The system's ability to correctly identify Supported or Conflicting Evidence claims remains largely untested.
...
With LongCLIP added to the retrieval pipeline, the system matched the claim image against stored image embeddings from reverse image search results, retrieving a web page that correctly identified the photograph as showing journalist Sheela Bhatt with Dawood Ibrahim. This enabled the VLM to produce the correct \textit{Refuted} verdict with a well-grounded justification: \textit{``The evidence states that the image does not show Supriya Shrinate but rather senior journalist Sheela Bhatt interviewing Dawood Ibrahim in 1987.''}

\subsection{Success Case: MMR Diversification Impact}

To illustrate the effect of MMR-based re-ranking on evidence quality, we examine Sample~139 from the validation set. The claim states that a woman shown in an image is the sister of Bageshwar Dham Dhirendra Shastri marrying a Muslim man. The ground truth identifies his sister as \textit{Rita Garg} and the woman in the image as actress Vandana Tiwari (Gehana Vasisth).
...
To illustrate a case where the VLM fails despite perfect retrieval, we examine Sample~136 from the validation set. The claim states: \textit{``A photo shows footwear that needs to be boycotted in 2023 because it depicts Lord Ganesha on them.''}
...
The ground truth verdict is \textit{Refuted} because the sandals were already withdrawn from the market in April 2003---twenty years before the claim was made. The claim misleadingly presents a resolved historical incident as a current issue requiring action in 2023.
