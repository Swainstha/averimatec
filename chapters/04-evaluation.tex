...
This chapter presents the experimental evaluation of the proposed fact-checking pipeline. It begins by describing the experimental setup and evaluation methodology. The chapter then presents the experimental results and discusses the research questions introduced in Chapter~\ref{ch:intro}. Finally, a discussion is presented along with qualitative examples and error analysis.

\section{Experimental Setup}
This section covers the experimental setup. The datasets are described, followed by relevant metrics.
All experiments were conducted on the University of Stavanger's UNIX computing infrastructure. Model indexing, retrieval, and inference were performed using NVIDIA A100 GPUs with 40GB and 80GB of GPU memory. GPU resources were accessed through the Slurm cluster management system.
...
Each claim is associated with a knowledge store containing pre-collected documents from which evidence must be retrieved. The knowledge store is organized into three modalities: (1)~\textit{Text} documents, consisting of web pages retrieved by search engines for the claim text; (2)~\textit{Image} documents, consisting of text extracted from web pages found via reverse image search on the claim image; and (3)~\textit{Image} documents, consisting of images through reverse image search. Table~\ref{tab:doc_stats} presents the document statistics.

The knowledge store exhibits extreme variance in document length. Text documents in the validation set range from a single word to over 2.4 million words, with a mean of approximately 5,700 words. This high variance necessitates document chunking strategies during retrieval indexing. The test split is substantially larger than the validation split, containing over 500,000 text documents compared to approximately 100,000 in validation, making efficient retrieval critical. However image related text documents for both sets are significantly smaller than the claim text related documents.
...
Validation-set experiments were evaluated locally using Qwen3-VL-8B-Instruct as the evaluation model. For the test set, predictions were submitted to the official AVerImaTeC evaluation server, which computed the final benchmark scores on the hidden test data. Unless otherwise stated, validation results refer to locally computed scores, while test results correspond to the official competition evaluation.
...
\subsection{RQ2: How does iterative question-answer generation compare to a Claim Based Retrieval (single pass) for QA generation in terms of evidence quality and verdict accuracy?}
...
On the test set (Table~\ref{tab:one_go_test}), the single pass approach with ICL achieves a question score of 0.622, evidence score of 0.312, verdict score of 0.327, and justification score of 0.297—representing our best test set submission.

Comparing it with the scores (with LongCLIP) of iterative approach of test set from Table ~\ref{tab:retrieval_ablation_test}, the single-pass approach consistently outperforms the iterative approach across all evaluation metrics. Question score increases from 0.495 to 0.622 (+25.7\%), evidence score from 0.224 to 0.312 (+39.3\%), verdict score from 0.230 to 0.327 (+42.2\%), and justification score from 0.170 to 0.297 (+74.7\%).

This results suggest that generating all questions and answers in a single coherent pass after evidence retrieval allows the VLM to maintain better contextual consistency and produce more holistic verification reasoning. The iterative approach, while theoretically allowing refinement across iterations, appears to suffer from error propagation—poor early questions lead to irrelevant evidence retrieval, which in turn degrades subsequent question generation.
...
fv(Factiverse)         & 0.4299 & 0.1614 & 0.1645 & 0.1179 \\
...
fv(Factiverse)        & 0.2885 & 0.1626 & 0.1591 & 0.1306 \\
Baseline & 0.5545 & 0.1707 & 0.1136 & 0.1322 \\
\hline
\end{tabular}
\end{table}

On the validation set, our system achieves the highest verdict score (0.81) among all teams, substantially outperforming the first-place VILLAIN system (0.6447) by over 16 percentage points. This demonstrates that our hybrid retrieval pipeline combined with single-pass VLM reasoning produces highly accurate verdict predictions when evaluated locally. Our justification score (0.42) also exceeds all systems except VILLAIN.
...
The final system configuration, consisting of hybrid multimodal dual-branch  retrieval, single-pass question-answer generation, and in-context learning, is referred to as HyMFact (Hybrid Multimodal Fact Checker) and is used for all official test-set comparisons.

On the test set, the performance gap between validation and test is notable: our best verdict score drops from 0.81 (validation, without ICL) to 0.327 (test, with ICL). This discrepancy suggests that the high validation verdict accuracy is partially inflated by the extreme class imbalance (90\% Refuted), and the test set likely contains a more balanced label distribution. Notably, our best test configuration uses ICL, which achieves lower verdict scores but generalizes better. Despite the val-to-test gap, our system outperforms the baseline across all metrics and achieves competitive evidence and justification scores relative to the proprietary MLLM-based systems, while using significantly fewer computational resources (a single 8B-parameter open-source VLM vs.\ multiple calls to commercial MLLMs).

HyMFact substantially improves upon the Factiverse baseline across all evaluation metrics. Question score increases from 0.2885 to 0.622, evidence score from 0.1626 to 0.312, verdict score from 0.1591 to 0.327, and justification score from 0.1306 to 0.297. This corresponds to relative improvements of 115.6\%, 91.9\%, 105.5\%, and 127.4\%, respectively. These gains demonstrate the effectiveness of the proposed retrieval enhancements, evidence reranking and diversification stages, and the transition from the original iterative framework to the final single-pass question-answer generation strategy.
...
\begin{itemize}
    ...
    \item \textbf{Disadvantages:} Lower question generation diversity (0.622 vs.\ 0.8897 for VILLAIN), reduced capacity for complex multi-hop reasoning, and smaller context window limiting the amount of evidence that can be processed simultaneously.
\end{itemize}
...
Without MMR, the top-ranked retrieved documents were redundant, all referencing the same incorrect source that named Vandana Tiwari as the sister. This led the QA module to extract an incorrect answer, which in turn left insufficient evidence to reach a verdict. With MMR diversification, the diversified document set included a BBC News article that correctly identified Rita Garg as the sister, enabling the system to extract the correct answer and produce the correct Refuted verdict.
